...
//public  static <T> String getToken(String endpoint, T payload)
    public  static String getToken(String endpoint, String filePath){
        if(token==null || isExpired()){
          token=  generateToken(endpoint,filePath);
        }
        return token;
    }
    ...
    public static String generateToken(String endPoint, String filePath){

        Response response = RestAssured.given()
                .contentType(ContentType.JSON)
                .body(JsonUtils.readJsonFile(filePath)).post(endPoint);
